...
constexpr inline static const ConsoleGameViewSettings defaultSettings =
    ConsoleGameViewSettings{};

static const std::string HeaderAlphabet{"abcdefghjklmnopqrstuvwxyz"};

ConsoleGameView::ConsoleGameView(std::shared_ptr<IGameModel> model)
    : ConsoleGameView(std::move(model), defaultSettings) {}

ConsoleGameView::ConsoleGameView(std::shared_ptr<IGameModel> model,
                                 const ConsoleGameViewSettings &settings)
    : ConsoleGameView(std::move(model), std::make_unique<TextCommandController>(
                                            readColumnNumber)) {}

ConsoleGameView::ConsoleGameView(
    std::shared_ptr<IGameModel> model,
    std::unique_ptr<TextCommandController> controller)
    : ConsoleGameView(std::move(model), std::move(controller),
                      defaultSettings) {}
...
ConsoleGameView::ConsoleGameView(
    ...
    std::unique_ptr<TextCommandController> controller,
    const ConsoleGameViewSettings &settings)
    : _settings(settings), _gridView(std::map<Coordinate, char>{}),
      _model(std::move(model)), _commandCallback(nullptr),
...
void ConsoleGameView::setCommandCallback(
    IGameView::TCommandCallback *callback) {
  if (nullptr != callback) {
...
void ConsoleGameView::drawWinScreen() {
  drawMap();
  std::cout << ConsoleColors::BrightGreen << "Yow win!" << ConsoleColors::Reset
            << std::endl;
}

void ConsoleGameView::drawLoseScreen() {
  drawMap();
  std::cout << ConsoleColors::Red << "You lose!" << ConsoleColors::Reset
            << std::endl;
}
...
void ConsoleGameView::drawMap() {
  ...
  std::cout << _settings.delimiter << _settings.delimiter
            << _settings.headerColor << std::flush;
  printHeader(grid.size(), _settings.delimiter);
  std::cout << _settings.resetColor << std::endl;

  int counter = 1;
  for (auto &row : grid) {
    std::cout << _settings.rowNumberColor << counter++ << _settings.resetColor
              << _settings.delimiter << std::flush;
    for (auto &item : row) {
      putCellChar(item);
