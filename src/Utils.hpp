...
template <typename T> static inline std::vector<T> tail(std::vector<T> vec) {
...
template <typename T> static T reduce(const std::vector<T>& vector, std::function<T(const T&, const T&)> functor) {
    T res{};
    for (auto& item : vector) {
